// reusing one song card 50 times for different songs!
  ...
  <div className='flex flex-col w-[250px] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer'>
    ...
    <div className='relative w-full h-56 group'>
      ...
      <div className={`absolute inset-0 justify-center items-center bg-black bg-opacity-50 group-hover:flex ${activeSong?.title === song.title ? 'flex bg-black bg-opacity-65' : 'hidden'} `} >
        ...
        {/* passing properties to play pause component */}
        <PlayPause 
          song={song}
    ...
    <div className='mt-4 flex flex-col'>
      ...
      <p className='font-bold text-lg text-red-50 truncate'>
        <Link to={`/songs/${song?.key}`}>
        {song.title}
        </Link>
      </p>

      <p className=' text-sm mt-1 text-gray-200 truncate'>
